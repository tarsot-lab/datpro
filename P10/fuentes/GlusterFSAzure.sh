...
# CPD (P10)
# Se pueden personalizar variables del tipo MOUNTPOINT, RAIDCHUNKSIZE, según necesidad.
# Aparte de Ubuntu y CentOS, puede modificarse para otras versiones de GNU/Linux.
...
scan_for_new_disks() {
    # Busca discos sin particionar
    declare -a RET
    DEVS=($(ls -1 /dev/sd*|egrep -v "${BLACKLIST}"|egrep -v "[0-9]$"))
    for DEV in "${DEVS[@]}";
    do
        # Chequea cada dispositivo para comprobar si hay una partición. 
        # De lo contrario, asume que no se ha particionado.
        if [ ! -b ${DEV}1 ];
        then
            RET+="${DEV} "
        fi
...
do_partition() {
# Esta función crea una partición primaria sobre el disco, usando todo el espacio disponible
    DISK=${1}
...
#
# Usamos el $PIPESTATUS, específico de bash, para asegurarnos que obtenemos el código de salida correcto.
